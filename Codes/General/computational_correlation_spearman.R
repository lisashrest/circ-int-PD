...
rownames(miRNA_expression) <- miRNA_expression$miRNA

miRNA_expression <- miRNA_expression[, details_up$filename]
miRNA_expression <- miRNA_expression %>%
  rownames_to_column("miRNA")

#import filteres circRNA normalized counts file
circRNA_expression <- read.csv("results/filtered_expression/PPMI_circ_logCPM.csv", header = T, stringsAsFactors = F, check.names = F, row.names = 1)

#add annotation to normalized counts
circ_anno <- read.csv("results/annotation/PPMI_circ_annotations.csv", header = TRUE)
circ_anno <- circ_anno %>%
...
write.csv(circRNA_expression, file = "results/filtered_expression/circRNA_correlation_input.csv")

miRNA_expression <- miRNA_expression[miRNA_expression$miRNA %in% valid_miRNAs,]
write.csv(miRNA_expression, file = "results/filtered_expression/miRNA_correlation_input.csv")
...
miRNA_for_row <- function(miRNA_expr_line, circRNA, circRNA_counts){
  ...
  cor_res <- cor.test(joined_counts$miRNA_counts, joined_counts$circRNA_counts, 
  ...
  res <- data.frame(circRNA = as.character(circRNA), miRNA = as.character(mirna), 
                    ...
                    spearman_R = as.numeric(corr_R), corr_pval = as.numeric(corr_pval))
